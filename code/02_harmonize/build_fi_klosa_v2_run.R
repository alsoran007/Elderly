...
PROJ <- "D:/AI_project/project3"; STAMP <- "2026-07-29"
...
RES <- file.path(PROJ,"results/fi_klosa_v2"); TAB <- file.path(RES,"tables")
...
MAP <- data.frame(stem=FI_STEMS,
 ...
 "w04C103","w04C106","w04C141","w04C148","w04C201","w04C203","w04C204","w04C205","w04C206","w04C068",
 "w04C209","w04C210","w04C214","w04C215","w04C217","w04C202","w04C211","w04C212","w04C213","w04C216",
 "w04C005","w04C081","w04C084","w04C208","w04C075","w04C076","w04C083","w04C001","w04C102","w04C056",
 "w04C105;w04C107","w04C143"),
...
MAP$coding_note <- c(rep("1=yes; 5=no; -8/-9 missing",9),
...
MAP$review_note <- c(rep("",9),"Other disease substitutes an unavailable disease slot.","Recent >5 kg weight change substitutes a metabolic symptom slot.","Two-week gloom substitutes a depressive-symptom slot.","Restless sleep substitutes a symptom slot.",rep("",28))
...
all_names <- names(read_dta(RAW,n_max=0)); id_name <- intersect(c("pid","PID","w04PID","w04pid"),all_names)[1]
...
if (length(setdiff(vars,all_names))) stop("Missing fields: ",paste(setdiff(vars,all_names),collapse=", "))
...
fi$other_disease <- yesno(d$w04C103); fi$weight_change <- ifelse(num(d$w04C106)>=1 & num(d$w04C106)<=4,1,ifelse(num(d$w04C106)==5,0,NA_real_))
fi$gloom_2wk <- ifelse(num(d$w04C141)%in%c(1,3),1,ifelse(num(d$w04C141)==5,0,NA_real_)); fi$restless_sleep <- freq4(d$w04C148)
for(i in 14:18) fi[[FI_STEMS[i]]] <- help(d[[MAP$source_vars[i]]]); fi$urina <- yesno(d$w04C068)
for(i in 20:24) fi[[FI_STEMS[i]]] <- help(d[[MAP$source_vars[i]]]); for(i in 25:29) fi[[FI_STEMS[i]]] <- help(d[[MAP$source_vars[i]]])
...
fi$mbmi <- ifelse(!is.na(bmi),ifelse(bmi<18.5 | bmi>=30,1,0),NA_real_); fi$concentration <- freq4(d$w04C143)
mat <- fi[,FI_STEMS,drop=FALSE]; mat[] <- lapply(mat,num)
fi$fi_n_valid <- rowSums(!is.na(mat)); fi$fi_n_found <- 41L; fi$fi_threshold <- 33L
fi$fi_full <- ifelse(fi$fi_n_valid>=33,rowSums(mat,na.rm=TRUE)/fi$fi_n_valid,NA_real_); fi$fi_excluded <- fi$fi_n_valid<33; fi$age_60_plus <- !is.na(fi$age) & fi$age>=60
stopifnot(all(fi$fi_full[!is.na(fi$fi_full)]>=0 & fi$fi_full[!is.na(fi$fi_full)]<=1))
MAP$n_nonmissing <- vapply(FI_STEMS,function(s) sum(!is.na(mat[[s]])),integer(1)); MAP$deficit_rate <- vapply(FI_STEMS,function(s) mean(mat[[s]],na.rm=TRUE),numeric(1)); MAP$deficit_rate[is.nan(MAP$deficit_rate)]<-NA_real_
...
write.csv(MAP,file.path(TAB,paste0("klosa_fi_v2_mapping_",STAMP,".csv")),row.names=FALSE,na=""); write.csv(summary,file.path(TAB,paste0("klosa_fi_v2_summary_",STAMP,".csv")),row.names=FALSE)
