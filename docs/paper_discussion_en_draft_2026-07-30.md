...
Sensitivity analysis SA-1 provides direct evidence for this interpretation. Restricting the index to the 19 deficits available in all six cohorts (FI_core) under a unified computability threshold left all four cross-cohort ΔC values below 0.02 (CLHLS −0.0044, HRS −0.0124, SHARE −0.0055, MHAS −0.0000). Cutting more than half the items therefore did **not** materially degrade discrimination. Had item availability been the binding constraint on transportability, SA-1 should have changed the conclusions substantially; it did not. This converges with H3: the operative constraint is what the same deficit label actually measures across differing questionnaire contexts, not the number of items or the number of training cohorts.

The practical implication for resource allocation in multi-centre ageing research is direct: rather than investing heavily in incorporating more cohorts into joint training, or in achieving full item coverage, it is more efficient to collect a small amount of data in the target population for L1 recalibration. L1 requires only the target population's baseline four-year mortality rate — a readily available figure — and demonstrated substantial calibration improvement (SHARE: IPA 0.028 → 0.114). This strategy deserves serious consideration in future international multi-cohort research frameworks.
...
(1) **FI items are not fully equivalent across cohorts**: MHAS had only 27 of 41 canonical stems; HRS was missing 8 items (including BMI and memory self-rating), substituted by documented proxy variables; dyslipidaemia and kidney disease used measurement-behaviour proxies rather than diagnosis variables. More consequentially, several items showed cohort-level positive rates that are difficult to attribute to genuine prevalence differences: **hypertension (`hibpe`) was 0.067 in KLoSA versus 0.682 in HRS, and arthritis (`arthre`) 0.025 versus 0.672** (Supplementary Table S2). Differences of this magnitude more plausibly reflect between-country variation in self-reported diagnosis rates and screening coverage than in true disease burden — meaning the same deficit code does not represent an equivalent health state across cohorts. Both items were retained in FI_full, since the Rockwood framework tolerates item heterogeneity, but their presence implies that absolute FI values should not be compared across cohorts at face value. Sensitivity analysis SA-1 partly mitigates this concern: restricting the index to items common to all six cohorts left discrimination essentially unchanged (all ΔC < 0.02), indicating that the conclusions do not hinge on the coding of the affected items. Measurement non-equivalence itself, however, cannot be removed statistically — it can only be reported transparently.
...
The study has five principal strengths. (1) **Scale and geographic coverage**: 76,074 complete-case participants aged ≥60 years and 10,790 four-year deaths across six countries/regions spanning East Asia, North America, Europe and Latin America — one of the largest cross-cultural FI mortality prediction studies to date. (2) **Pre-registration and outcome blinding**: All analytic decisions were locked in the SAP before any outcome–predictor analyses (commit 9bc7b85), with a complete decision audit trail (D-001 to D-036, including SAP amendment A-001) satisfying the TRIPOD+AI and PROBAST-AI audit-trail requirements. (3) **Attribution-decomposition recalibration framework**: The L0–L3 ladder decomposes performance loss into attributable event-rate drift (L1), slope drift (L2) and parameter heterogeneity (L3), providing an operationally useful methodological reference for cross-cultural model transfer research. (4) **Item-level auditability of the harmonisation route**: FI construction for CHARLS and KLoSA was re-implemented from the official Gateway `.do` scripts, with all 41 items carrying the line number of their defining statement (Supplementary Table S1), so each can be verified against the official source. (5) **Open data and full reproducibility**: All six cohorts are available through official application; the full analytic codebase is publicly archived on GitHub with complete version-controlled reproducibility.
