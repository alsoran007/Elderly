...
H4 further quantifies the attributable sources of calibration failure. A simple L1 intercept update — adjusting the intercept using only the target-cohort event rate, requiring no re-collection of predictor data or model retraining — explained approximately 95% of recoverable IPA gain in SHARE (0.028 → 0.114) and 89% in HRS (0.193 → 0.209). For most target cohorts, a single lightweight recalibration is therefore sufficient to raise a cross-cultural model's calibration performance to a practically useful level, at very low operational cost.
...
(3) **Education excluded from main analysis**: Due to concerns about the reliability of harmonised education variables, the primary model included only age, sex and FI. Residual confounding by socioeconomic status cannot be excluded; this is a preliminary version and education adjustment is planned prior to submission.
