...
**Word count**: Abstract 331 (structured body, excluding keywords); main text 6,901 (Introduction 485, Methods 2,127, Results 2,322, Discussion 1,967; excludes tables, declarations, references and figure list)
...
Discrimination was well preserved: C-index 0.8389 (bootstrap 95% CI 0.8301–0.8481). Calibration was not: O:E ratio 1.2473, calibration intercept 0.5976, calibration slope 0.9393, with Brier score 0.1751 and IPA 0.2957. The frozen model therefore ranked risk well while systematically under-predicting its absolute level.

The demographic-only Model A sharpens both halves of that result (Table 2 for coefficients; Figure 3A for ROC curves). On discrimination, Model A reached C = 0.8051 in CLHLS, so FI contributed ΔC = 0.0339 externally against 0.0351 in development — a gap of 0.0012, indicating that FI's incremental value is not confined to the sample that selected it. On calibration the comparison inverts: Model A was better calibrated than Model B (O:E 1.044 vs. 1.247; mean predicted risk 0.443 vs. 0.371 against an observed 0.463) while remaining worse on overall accuracy (IPA 0.273 vs. 0.296). Discrimination and calibration therefore moved in opposite directions within this cohort pair; the mechanism is examined in §4.3. Because H2 as pre-registered concerns only the relative magnitude of decay in the two dimensions, this directional inversion is an additional observation rather than part of the H2 test.

Under-prediction was concentrated in the middle of the risk distribution (Figure 3B): predicted versus observed mortality was 4.0% vs. 4.9% at decile 1, 23.8% vs. 40.3% at decile 5, and 47.2% vs. 65.9% at decile 7, converging at both extremes. This is the expected signature of transporting a model from a low-event-rate to a high-event-rate cohort.
...
The Aim 1 comparison between Model A and Model B makes the point in its strongest form. Within one cohort pair, adding FI improved discrimination (C 0.805 → 0.839) while worsening calibration (O:E 1.044 → 1.247). The two dimensions did not merely decay at different rates; they moved in opposite directions. CLHLS is an oldest-old cohort, so an age-and-sex model is driven towards high predicted risk by the age distribution alone and lands near the observed rate almost incidentally. Adding FI improves ranking but lowers the absolute level, because CLHLS combines high mortality with a *lower* median FI than the development cohort (0.169 vs. 0.200): the FI term subtracts from predicted risk in a population that in fact dies more often. Any evaluation resting on a single summary statistic would have missed this — a study reporting only C-index would have recorded an unambiguous improvement, one reporting only O:E a deterioration, and both would have been describing the same model. Model B nonetheless remains the better choice on ranking and overall accuracy, and its calibration deficit is exactly what a single intercept update corrects.
...
*Word count: Abstract 331 | Introduction 485 | Methods 2,127 | Results 2,322 | Discussion 1,967 | main text total 6,901 (excluding tables, declarations, references and figure list)*
