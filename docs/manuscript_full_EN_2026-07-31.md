...
The demographic-only Model A sharpens both halves of that result (Table 2 for coefficients; Figure 3A for ROC curves). On discrimination, Model A reached C = 0.8051 in CLHLS, so FI contributed ΔC = 0.0339 externally against 0.0351 in development — a gap of 0.0012, indicating that FI's incremental value is not an artefact of the sample that selected it. On calibration the comparison inverts: Model A was **better** calibrated than Model B (O:E 1.044 vs. 1.247; mean predicted risk 0.443 vs. 0.371 against an observed 0.463) while remaining worse on overall accuracy (IPA 0.273 vs. 0.296). The mechanism is that CLHLS is an oldest-old cohort, so an age-and-sex model is driven towards high predicted risk by the age distribution alone and lands near the observed rate almost incidentally; adding FI improves ranking but lowers the absolute level, because CLHLS has a *lower* median FI than the development cohort (0.169 vs. 0.200) despite far higher mortality. Discrimination and calibration are thus governed by different quantities and can move in opposite directions within a single cohort pair. H2 as pre-registered concerns only the *relative magnitude* of decay in the two dimensions (§3.6, §4.3); this directional inversion is an additional observation beyond that hypothesis, not part of its test. Model B remains preferable on ranking and overall accuracy, and its calibration deficit is what a single intercept update corrects.
...
The Aim 1 comparison between Model A and Model B makes the point in its strongest form. Within one cohort pair, adding FI improved discrimination (C 0.805 → 0.839) while *worsening* calibration (O:E 1.044 → 1.247). The two dimensions did not merely decay at different rates; they moved in opposite directions. The reason is that CLHLS combines a high mortality rate with a *lower* median FI than the development cohort (0.169 vs. 0.200), so the FI term subtracts from predicted risk in a population that in fact dies more often, while age alone happens to approximate the observed rate. Any evaluation resting on a single summary statistic would have missed this: a study reporting only C-index would have recorded an unambiguous improvement, and one reporting only O:E would have recorded a deterioration. Both would have been describing the same model.
